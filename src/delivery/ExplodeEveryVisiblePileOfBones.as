package delivery
{
	import com.headchant.asciipanel.AsciiPanel;
	import delivery.Explosion;
	import effects.Effect;
	import effects.MagicDamage;
	
	public class ExplodeEveryVisiblePileOfBones implements Animation
	{
		public var world:World;
		public var fov:FieldOfView;
		public var magicEffect:Effect;
		
		public var explosions:Array = [];
		public var isFirstTick:Boolean = true;
		
		public function ExplodeEveryVisiblePileOfBones(creature:Creature, magicEffect:Effect) 
		{
			this.world = creature.world;
			this.fov = new FieldOfView();
			this.magicEffect = magicEffect;
			
			fov.calculateVisibility(creature.x, creature.y, creature.viewDistance - 1, function(vx:int, vy:int):Boolean {
				return creature.world.getTile(vx, vy).allowsVision;
			});
		}
		
		public function get isDone():Boolean 
		{
			return !isFirstTick && explosions.length == 0;
		}
		
		public function tick(terminal:AsciiPanel):void 
		{
			if (isFirstTick)
				firstTickAction(terminal);
			else
				continueExplosions(terminal);
		}
		
		private function firstTickAction(terminal:AsciiPanel):void 
		{
			isFirstTick = false;
		
			for (var x:int = 0; x < 80; x++)
			for (var y:int = 0; y < 80; y++)
			{
				if (!fov.isVisibleNow(x, y))
					continue;
				
				if (world.hero.canSeeLocation(x, y))
					terminal.write(" ", x, y, magicEffect.primaryColor, magicEffect.secondaryColor);
				
				var item:Item = world.getItem(x, y);
				if (item is PileOfBones)
				{
					terminal.write(" ", x, y, 0xffffffff, 0xffffffff);
					
					world.removeItem(item);
					explosions.push(new Explosion(world, x, y, 49 * 2, magicEffect));
				}
			}
		}
		
		private function continueExplosions(terminal:AsciiPanel):void 
		{
			var nextExplosions:Array = [];
			
			for each (var explosion:Explosion in explosions)
			{
				explosion.tick(terminal);
				
				if (!explosion.isDone)
					nextExplosions.push(explosion);
			}
			
			explosions = nextExplosions;
		}
	}
}